...
import com.frontierwholesales.core.services.NavigationConfig;
			...
			if (rootRes != null) {
				...
				while (children.hasNext()) {
					...
					if (!child.isHideInNav()) {
						...
						writer.value(i);
						writer.key("route");
						if(child.getVanityUrl() != null){
                            writer.value(child.getVanityUrl());
                            logger.info("vanity url found: " + child.getVanityUrl());
                        }else{
                             writer.value(child.getPath());
                            logger.info("no vanity url found: " + child.getPath());
                        }
						writer.key("type");
						...
						if (childIterator.hasNext()) {
							...
							while (childIterator.hasNext()) {
								...
								if (!childPage.isHideInNav()) {
									...
									writer.value(j);
									writer.key("route");
									if(childPage.getVanityUrl() != null){
                                        writer.value(childPage.getVanityUrl());
                                        logger.info("vanity url found: " + childPage.getVanityUrl());
                                    }else{
                                        writer.value(childPage.getPath());
                                        logger.info("no vanity url found: " + childPage.getPath());
                                    }
									writer.key("type");
									writer.value("ContentPageComponent");
									writer.key("params");
